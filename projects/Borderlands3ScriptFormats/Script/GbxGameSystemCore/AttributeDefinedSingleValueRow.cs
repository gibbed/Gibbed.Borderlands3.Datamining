...
{
    public class AttributeDefinedSingleValueRow : PropertyBag
    {
        #region Fields
        private AttributeInitializationData _Value;
        #endregion

        #region Properties
        public AttributeInitializationData Value
        {
            get => this._Value;
        ...
        protected override bool SerializeProperty(IUnrealSerializer serializer, ref PropertyTag tag)
        {
            if (tag.Name == "Value")
            {
                serializer.Serialize(ref this._Value);
                return true;
            }
